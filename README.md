...
* **`math_functions.s`**: Implementación de las funciones aritméticas en ensamblador ARM64.
* **`wrapper.c`**: Puente de conexión en C para exportar símbolos a Python.
* **`main.py`**: Lógica principal, invocación de librería y medición de tiempos.
* **`Makefile`**: Automatización de la compilación de la librería compartida `.so`.
